...
@app.get("/")
def home(request: Request, is_auth=Depends(full_auth_check)):
    if is_auth:
...
@app.get("/register")
def register_user(request: Request, is_auth=Depends(token_authenticate_user)):
    if is_auth:
...
def register_page(
    ...
    user = get_user_from_db(FAKE_DB, login_input)
    if user is None and (len(login_input) > 0 and len(pass_input) > 0):
        insert_new_user_in_db(FAKE_DB, login_input, pass_input)
        return RedirectResponse(url="/login", status_code=302)
    return RedirectResponse(url="/register", status_code=302)


@app.get("/login")
def login_user(request: Request, is_auth=Depends(token_authenticate_user)):
    if is_auth:
        return RedirectResponse(url="/", status_code=302)
...
def login_page(
    ...
    if user:
        ...
        response.set_cookie("refresh_token", refresh_token, expires=2592000, httponly=True)
        return response
    return RedirectResponse(url="/login", status_code=302)
...
@app.get("/refresh")
def refresh(is_auth: dict | bool = Depends(token_authenticate_user)):
    login_from_token = is_auth.get("username")
    user = get_user_from_db(FAKE_DB, login_from_token)
    access_token = create_access_token(user)
